...
name: testrrrr
description: >
  通过调用 Prana 平台上的远程 agent 完成以下处理：接受用户输入并执行对应功能
...
技能使用场景:在投研场景中处理用户输入并执行对应分析任务
例如：请帮我执行testrrrr功能、我想用testrrrr处理输入数据
...
脚本参数 `-q`：用户要执行的需求任务（例如：请帮我执行testrrrr功能、我想用testrrrr处理输入数据）。不要包含账号密码、付款信息、个人隐私等敏感内容。
...
- **Node.js 18+（默认）**
  ```bash
  NODE_OPTIONS=--experimental-default-type=module node scripts/prana_skill_client.js -q "请帮我执行testrrrr功能"
  ```
- **Python 3（备选）**
  ```bash
  python3 scripts/prana_skill_client.py -q "请帮我执行testrrrr功能"
  ```
